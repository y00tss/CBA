...
async def test_document_process_success():
    ...
    # mock_doc.get_updated_document = AsyncMock(return_value=f"{TEMP_DIR}/document.docx")
...
async def test_document_process_failure():
    ...
    DocumentWorkFlowFactory.create_workflow = AsyncMock(side_effect=Exception("Document processing failed"))
    pass
